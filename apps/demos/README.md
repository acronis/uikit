...
Shared **source-only** demo components consumed by `apps/demo` via the
...
yet wired into the demo app), add it as a sibling workspace rather
than retrofitting a `dev` script here — the no-dev contract is what
lets the source stay drop-in consumable.
