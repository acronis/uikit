...
<Callout type="info">
  The source SVG package (`icons-svg`), the tooling under `tools/`, and the apps
  (`demo`, `docs`, `kitchen-sink`) are **private** and not published.
  `@acronis-platform/shadcn-uikit` (`ui-legacy`) is also published, but it's
  frozen for new work as part of the `ui-react` migration and isn't documented
  on this site.
</Callout>
