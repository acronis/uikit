...
Each chip's remove-control accessible label defaults to `"Remove <key>
filter"` — override via `getRemoveFilterLabel(key)` to localize it. Its
...
Group fields with a divider as a layout convention if desired. Calling
...
[Filter popover](/components/filter-popover) pattern for the hand-rolled
`Button` + `Popover` + `InputSelect` composition.

> **Design-pending.** `FilterSearch`/`FilterSearchActions` have a real Figma
> node (see the API reference below), and `FilterSearchAppliedFilters`'s chip row
> now does too — it landed as its own
> [`FilterChips`](/docs/components/filter-chips) component, which
> `FilterSearchAppliedFilters` renders through. `FilterSearchFilters` still has
> no node: it themes entirely from shared semantic tokens plus its composed
> components' own tiers (`Button`, `Popover`), and will be reconciled against
> the real design once a mockup lands.
