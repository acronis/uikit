...
`DropdownMenu` (root) accepts the Base UI Menu root props **except `modal`** —
`open` / `defaultOpen`, `onOpenChange`. `DropdownMenuItem` / `DropdownMenuLabel` /
...
surface of each part, keeping the `modal` exception below in mind.

<Callout type="warn" title="modal is not configurable">
  Base UI's `Menu.Root` defaults to `modal={true}`, which locks document scroll
  and renders an interaction-blocking backdrop. A dropdown menu is a
  light-dismiss overlay, not a modal surface, so `DropdownMenu` always renders
  `modal={false}` and `modal` is omitted from `DropdownMenuProps` — passing it
  is a type error. The menu therefore never scroll-locks the page and never
  blocks pointer events outside the popup; it light-dismisses on outside press
  or `Escape`. Reach for `Dialog` or `Sheet` when you need genuinely modal
  behaviour.
</Callout>
