...
(`average: 'desktop'`) or of every plotted series (`average: true`) — and nothing
is drawn when there are no numeric values to average. The rule extends the axis
domain, so a target above the data maximum stays visible instead of being
clipped. `BarChart` and `AreaChart` take the same config.
