...
- `{ side: 'a' | 'b' }` — diverging palette only. Pins the series to one of the
  two hue families. All `side: 'a'` series share the a-hue stops (a3 → a2 → a1,
  strongest-first), and all `side: 'b'` series share the b-hue stops likewise.
  Series with no `side` walk the default interleaved ramp, skipping any stop
  already claimed by a sided series.
...
[CategoryBar](/components/category-bar),
[BarChart `orientation="horizontal"`](/components/bar-chart#horizontal),
...
recharts' content props; the legend additionally takes `fontSize`
(`'xs' | 'sm' | 'base' | 'lg' | 'xl'`, default `'xs'`), which sizes its labels
and the `list` variant's values. The per-type chart components expose the same
option as `legendFontSize`.
