...
always rendered below the chart; `legendFontSize` sizes its text (`'xs'` by
default) — a larger size grows the band the plot is centred above, so pair it
with a taller chart height if needed.
