...
coordinates itself, so the plot does not mirror even when the page does. For a radar that means the web's `startAngle` / `endAngle` sweep, the order
the categories are placed in, and the spoke and radius-axis labels around them all
