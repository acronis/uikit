...
> Not backed by a shared design-system design. This is the pattern extracted
> from the "Service status" list in the product-specific Cyber-Compliance
> Figma file, not from the shared ui-react file — its Figma mapping is a
> product mapping, not a design-system source of truth. Whether to formalize
> this as a generic kit component (with its own shared-file design, variants,
> and states) is still an open decision.
