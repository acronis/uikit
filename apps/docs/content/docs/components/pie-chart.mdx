...
The legend is a right-hand list — one row per slice with its value — laid out
beside the plot rather than inside it. `legendFontSize` sizes its labels and
values (`'xs' | 'sm' | 'base' | 'lg' | 'xl'`, default `'xs'`),
`legendValueFormatter` formats the values. `margin` sets the plot-area
