...
`legendPos` moves the legend to the top edge, and `margin` sets the plot-area
...
`labelFormatter` still applies, but only to the numeric part, so a label and its
tooltip stay in the same units:
...
no longer applies in that mode:
