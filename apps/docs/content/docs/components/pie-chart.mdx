...
into has no accessible name — recharts emits an empty `<title>` / `<desc>` pair and
nothing else. It does switch its own accessibility layer on for this chart type, so
the plot is a tab stop carrying `role="application"`: a focus stop that announces
...
coordinates itself, so the plot does not mirror even when the page does. For a pie that means the sweep still runs from `startAngle` to `endAngle`
the same way in both directions, and the slice labels, their leader lines and the
`centerLabel` stay where the geometry puts them.
