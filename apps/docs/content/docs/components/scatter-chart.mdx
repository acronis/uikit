...
re-points it within the palette. `ScatterChart` resolves them into the
`--color-<key>` custom properties its points fill from.
...
`showTooltip` / `showLegend`. `legendFontSize` sizes the legend text (`'xs'`
by default):
