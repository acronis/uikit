...
it and focusing it opens the tooltip on the first point. The left/right arrow
keys step it from point to point. Enter *toggles* the tooltip rather than
...
`ScatterChart` passes none of them down, because it spreads the props it doesn't
...
plain markup with no `role="status"` or `aria-live`, so the coordinates it
...
the slope of a cloud of points would reverse with it. This is what mainstream
charting libraries do by default.
...
rotated X-axis ticks drift. Text anchored `middle` is direction-immune either
way: unrotated X-axis ticks and axis titles. Because the pin stops at the surface,
