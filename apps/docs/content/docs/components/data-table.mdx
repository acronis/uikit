...
The TanStack types DataTable's props reference are re-exported from
`@acronis-platform/ui-react`, so typing columns and state handlers needs no
direct `@tanstack/react-table` import: `Column`, `ColumnDef`, `Row`,
`OnChangeFn`, `TanstackTable` (TanStack's `Table`, aliased so it doesn't clash
...
`DataTableColumnHeader` can type its own props. You still need
`@tanstack/react-table` itself for `useReactTable` and the row-model factories
when you build an external instance.
...
| Component                | Props                                                                                                                  |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------- |
| `DataTableColumnHeader`  | `column` (required), `title` (required), `sortLabel` (default `` (title) => `Sort by ${title}` ``), plus any `button` attribute. |
