...
`iconOnly`), not in the toolbar, keeping sorting to one click.
...
drag-and-drop over TanStack's `columnOrder`; the grab/grabbing cursors come from
the `--ui-draggable-cursor[-active]` tokens, the counterpart of the resize
handle's `--ui-resizable-cursor`). Pinned columns are excluded — they're anchored
to a table edge by definition.
