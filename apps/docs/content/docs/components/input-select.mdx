...
the corresponding Base UI `Select` primitive props — except that
`InputSelectSection` and `InputSelectSectionLabel` omit `render`, since
overriding it would silently drop the section's base styling. `InputSelect`
accepts
