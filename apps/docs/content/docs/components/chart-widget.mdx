...
placeholder come from one prop rather than two. `stateMessage` overrides the
placeholder's default label.
...
lives in what it holds. `Card` exposes a `render` prop, so a dashboard that
wants each widget to be a landmark can render it as a `<section>`.
