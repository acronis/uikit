# DialogFooterCarousel2 — behavior

DialogFooterCarousel2 is a pure function of its props — no internal state.
The carousel engine (which slide is active, how to move between slides) lives
in the caller — typically `DialogWelcome2`'s Embla instance — which threads
...
Scenario: First slide (variant="start")
  Given a DialogFooterCarousel2 with variant="start"
...
Scenario: A middle slide (variant="middle")
  Given a DialogFooterCarousel2 with variant="middle"
...
Scenario: The last slide (variant="end")
  Given a DialogFooterCarousel2 with variant="end"
  Then `Back` is rendered in boxLeft
...
Scenario: Jumping to an arbitrary slide
  Given a DialogFooterCarousel2 with slideCount=3
