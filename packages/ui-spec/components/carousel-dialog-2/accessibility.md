...
- Each dot in the indicator is a real `<button>` with an explicit
  ...
  (e.g. `Pagination`, `Tabs`).
- CarouselDialog2 is a plain layout container — no landmark role of its own.
  It's meant to live inside `DialogFooterCarousel2` inside a dialog (e.g.
  `DialogWelcome2`), which owns the `role="dialog"` / focus-trap semantics.
