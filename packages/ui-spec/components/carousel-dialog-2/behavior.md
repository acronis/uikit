# CarouselDialog2 — behavior

CarouselDialog2 is a pure function of its props — no internal state. The
carousel engine (which slide is active, how to move between slides) lives in
the caller — typically `DialogWelcome2`'s Embla instance, threaded through
`DialogFooterCarousel2` — which passes `slideCount`/`selectedIndex` in and
receives `onSelectIndex`/`onBack`/`onNext`/`onPrimaryAction` back out.

```gherkin
Scenario: First slide (variant="first")
  Given a CarouselDialog2 with variant="first"
...
Scenario: A middle slide (variant="middle")
  Given a CarouselDialog2 with variant="middle"
...
Scenario: The last slide (variant="last")
  Given a CarouselDialog2 with variant="last"
  Then `Back` is rendered in boxLeft
...
Scenario: Jumping to an arbitrary slide
  Given a CarouselDialog2 with slideCount=3
...
Scenario: A single-slide carousel
  Given a CarouselDialog2 with slideCount=1
