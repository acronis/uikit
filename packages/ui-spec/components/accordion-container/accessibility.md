...
- The trigger renders only an icon (no text) — the consumer composing it into
  a header must give it an accessible name via `aria-label` or
  `aria-labelledby` pointing at the surrounding heading/title.
...
The trigger's chevron uses `--ui-glyph-on-surface-primary`, which meets WCAG AA
against a surface background in both light and dark themes. AccordionContainer
adds no other color; contrast for everything else is governed by whatever the
consumer places inside it.
