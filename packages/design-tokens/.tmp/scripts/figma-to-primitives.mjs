...
// Skip an orphan no longer present in the snapshot meta: if nothing references
// it (the exporter only resolves orphans that are alias targets), it doesn't
// need emitting, and a stale hardcoded ID must not hard-crash the pull. Update
// this list when the Figma Transparent/Inverted stops are re-spaced.
const orphanMeta = loadMeta();
for (const o of ORPHAN_PALETTE) {
  const m = orphanMeta[o.variableId];
  if (!m) {
    console.warn(`figma-to-primitives: ORPHAN_PALETTE ${o.variableId} (palette.${o.ourPath.join('.')}) absent from snapshot meta — skipping (not referenced in this Figma export).`);
    continue;
  }
  const ext = {
    'com.figma.scopes': m.scopes ?? [],
    'com.figma.variableId': o.variableId,
  };
  if (m.hidden) ext['com.figma.hiddenFromPublishing'] = true;
