...
| **VISIBLE** | `-` for a normal component. For one marked `visibility: internal` and/or `deprecated` in `index.yaml`: not referenced in `packages/ui-react/src/index.ts`'s barrel, its docs `.mdx` frontmatter declares `internal`/`deprecated.replacement`, and its Storybook `tags` array includes `'internal'`/`'deprecated'`. This is the static preview of `packages/ui-spec/__tests__/visibility.test.ts`, which is the CI-authoritative check. | **Yes** → DRIFT                |
