...
- The recharts source you port from: `apps/demo/src/components/<X>ChartPlayground.tsx`,
  **archived** — `apps/demo` was removed from the repo. Read it out of the last
  commit before removal, `2e257c95` (also tagged `ui-legacy-final`, if that tag
  has been mirrored to your remote):
  `git show 2e257c95:apps/demo/src/components/<X>ChartPlayground.tsx`.
...
does. It is archived (see above), so read it out of that commit, plus the
shared primitives' story:

```bash
git show 2e257c95:apps/demo/src/components/<ChartName>Playground.tsx   # or the name from the table
