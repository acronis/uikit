...
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `index.yaml`       | `status: draft`, **no `figma:` block**, `category: Data Display`. `visibility`/`deprecated` are normally omitted for a new chart — see `/component-readiness`'s `VISIBLE` column and `packages/ui-spec/__tests__/visibility.test.ts` if either ever applies. |
