...
| OKLch→sRGB→HSL math + per-brand emit loop          | `ui-legacy/scripts/generate-white-label-themes.ts` (archived)  | **Blueprint** for any conversion helper; the new pipeline emits rgb `light-dark()` from Style Dictionary instead.                                             |
...
| `ui-legacy` four shipped themes                    | `packages/ui-legacy/src/styles/themes/` (archived)             | Naming/precedent for brand entry points; the `acronis-white-label.scss` shows the per-brand-class delivery we're replacing.                                   |
