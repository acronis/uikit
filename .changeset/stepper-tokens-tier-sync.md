...
Consumers composing their own `Avatar` as a step's marker should now add `className="[box-shadow:none]"` to it — Avatar's default outset ring otherwise shows as an unwanted halo on a filled step container.
