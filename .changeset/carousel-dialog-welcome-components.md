...
- `Carousel`: an internal, Embla-driven slider (root, content, item, previous/next controls, `useCarousel`). **Not** part of the public API — a design-pending v1 kept under Storybook's `Internal/` group, meant only to back `DialogWelcome`'s carousel layout. `CarouselItem` and the `CarouselApi` type stay exported since `DialogFooterCarousel`'s own stories pair it with a bare `Carousel`.
