...
The root's prop surface mirrors Figma's own component property list exactly (`variant` + the `Feed` slot only, verified via Code Connect). The variant-switch actions (Maximize/Minimize/Collapse chat, New chat) render as inert buttons pending a product decision on the interaction model for moving between variants — see the open questions in `packages/ui-spec/components/ai-chat/README.md`.
