...
- `packages/icons-sprite/` — published SVG sprites (combined / monocolor /
  ...
  changing `icons-svg` sources. Monocolor symbols are `currentColor`-themable.
...
- `tools/style-dictionary/` — a Style Dictionary v5 translation pipeline
  ...
  `dist/`.
- `tools/figma-design-assets-sync/` — exports `_assetsource/*` icon SVGs from a
  Figma frame, SVGO-optimizes them, and regenerates the matching
  `packages/design-assets` pack (`packs/<pack>/*.svg` + `packs/<pack>.json`).
  Run via `tsx` (no build step) from the consumer package:
  `pnpm --filter @acronis-platform/design-assets sync`. The write is diff-gated —
  it prints added/modified/deleted and reverts unless approved (`--dry-run`,
  `--yes`).
